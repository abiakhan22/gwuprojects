...
int sys_lk_create(void){
	int fd;
	if (argint(0, &fd) < 0) return -1;
	if(fd==1){
		return lock_create(LOCK_SPIN);
	}
	if(fd==2){
		return lock_create(LOCK_BLOCK);
	}
	if(fd==3){
		return lock_create(LOCK_ADAPTIVE);
	}
	return -1;
}

int sys_lk_take(void){
	int fd;
	if (argint(0, &fd) < 0) return -1;
	return lock_take(fd);
}

int sys_lk_release(void){
	int fd;
	if (argint(0, &fd) < 0) return -1;
	return lock_release(fd);
}

int sys_lk_delete(void){
	int fd;
	if (argint(0, &fd) < 0) return -1;
	lock_delete(fd);
	return 0;
