...
void
child(int lockid, int pipefd, char tosend, int lockidII)
{
	int i, j;
...
//LVL4
  if ((lockidII = LOCK_CREATE(LOCK_BLOCK)) < 0) {
    printf(1, "Lock creation error\n");
    exit();
  }

	for (i = 0 ; i < NCHILDREN ; i++) {
		if (fork() == 0) child(lockid, pipes[1], data[i], lockidII);
	}
	close(pipes[1]);
