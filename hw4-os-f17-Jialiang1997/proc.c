...
fork(void)
{
	...
	np->parent = curproc;
	*np->tf    = *curproc->tf;
	np->ifprocessorthread = 0; //process
...
exit(void)
{
	...
	//Thread
	if (curproc -> ifprocessorthread != 0) panic("It is a thread not a process!");
...
found:
	...
	// Allocate kernel stack.
	if ((p->kstack = stack) == 0) {// passing the stack value into bottom function
		p->state = UNUSED;
		return 0;
	}
	sp = p->kstack + KSTACKSIZE;
...
int
gwthd_fork2(gwthd_t *childid, gwthd_fn_t fn, void *data, void *stack)
{
	int i, pid;
	struct proc *np;//child pointer
	struct proc *curproc = myproc();//parent pointer
	...
	if ((np->pgdir = curproc->pgdir) == 0) {
		//the orignial code is copying the pgdir value within parent(dont change the parent)
		//the new code is make the pointer point to the pgdir (share the virtual adddress space)

		//kfree(np->kstack); <--do this in ulib.c
		np->kstack = 0;
		np->state  = UNUSED;
		return -1;
	}
	*/
	...
	np->parent = curproc;
	*np->tf    = *curproc->tf;
	np->ifprocessorthread = 1;//===================lvl6 is thread
	...
	//passing the child id
	*childid = pid;

	return pid;//when it is parent return pid
						 //when it is child return 0
...
newexit(void)
{
	...
	if (curproc == initproc) panic("init exiting");
	//Process
	if (curproc -> ifprocessorthread == 0) panic("It is a process not a thread!");
	// Close all open files.
	for (fd = 0; fd < NOFILE; fd++) {
		if (curproc->ofile[fd]) {
			fileclose(curproc->ofile[fd]);
...
newwait(gwthd_t child)
{
	...
	for (;;) {
		...
		havekids = 0;
		for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
			// to check the one i want to wait is child i need cuz the parent can have multi children
			if (p->parent != curproc && p->pid != child) continue;
			havekids = 1;
			if (p->state == ZOMBIE) {
				// Found one.
				pid = p->pid;
