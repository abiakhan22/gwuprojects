...
int sys_g_create(void){
	gwthd_t *childid;
	gwthd_fn_t fn;
	void *data;
	void *stack; //store the user lvl stack

	// get it 4 times for each pointer
	if (argptr(0, (void *)&childid, sizeof(childid)) < 0) return -1;
	if (argptr(1, (void *)&fn, sizeof(fn)) < 0) return -1;
	if (argptr(2, (void *)&data, sizeof(data)) < 0) return -1;
	if (argptr(3, (void *)&stack, sizeof(stack)) < 0) return -1;

	return gwthd_fork2(childid, fn, data, stack); //passing data in proc.c
}

void    sys_g_exit(void){return newexit();}

int     sys_g_join(void){// child is void
	gwthd_t child;
	if (argint(0, &child)  < 0) return -1;
	//pass the value of child
	int i = newwait(child);
	if(i > 0) return 0;
	return -1;
}

gwthd_t sys_g_id(void){return gwthd_idhelper();}
